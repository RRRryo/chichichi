...
class FunctionsV3
{
    ...
    {    	
		...
		if (isset($getdata['filter_delivery_type'])){			
			switch ($getdata['filter_delivery_type']) {
				case 8:
					$and = "AND ( service='1' OR service ='2' OR service='5' OR service='7')";
					break;
				case 9:
					$and = "AND ( service='1' OR service ='3' OR service='6' OR service='7')";
					break;
				case 10:
					$and = "AND ( service='4' OR service ='5' OR service='6' OR service='7')";
					break;
				/*case 1:
	...
	public static function getFreeDeliveryTagAlignCenter($merchant_id='')
	{
		$fee=getOption($merchant_id,'free_delivery_above_price');
		$metro_fee=getOption($merchant_id,'free_metro_delivery_above_price');
		$html = '';
    ...
    public static function getMerchantPaymentList($merchant_id='')
    {
		...
		if ( Yii::app()->functions->isMerchantCommission($merchant_id)){
			...
			$payment_available[]='itp';
			$payment_available[]='wcp';
    ...
    public static function getLanguage()
    {
    	...
    	if ($list=Yii::app()->functions->getLanguageList()){
    		foreach ($list as $val) {
    	//$flag = Yii::app()->functions->getFlagByCode_languale($val['country_code'])." ".ucwords($val['language_code']);

...
//echo $res = Yii::app()->functions->getRecentMaxOrders($merchant_id);
//dump($res);
...
/*  $timestamp =strtotime('2016-12-30');
 $day_of_week = date('l', $timestamp);
 echo  $today  = lcfirst($day_of_week);

 die;*/
/*  $DbExt=new DbExt;
  $startdate = '2017-01-04 07:15';
 $enddate = '2017-01-04 08:15';
 $start = '07:15';
 $end = '08:15';
 $date = '2017-01-04';
 $time = '2017-01-04 08:00';
 if((strtotime($startdate) < strtotime($time)) && (strtotime($enddate) > strtotime($time))) 
			         {
					 echo $stmt="SELECT count(order_id) as max_order from
						   {{order}}
						   WHERE merchant_id='$merchant_id' AND 
						   delivery_date = '".$date."' AND
						   delivery_time BETWEEN '".$start."%' AND '".$end."%'
						   ";
						   /* $stmt="SELECT count(order_id) as max_order from
						   {{order}}
						   WHERE merchant_id='$merchant_id' AND 
						   delivery_date = '".$date."' AND
						   delivery_time < '".$start."%' AND  delivery_time   '".$end."%'
						   ";
						   $test = 'test';*/
  ...
  if ( $res=$DbExt->rst($stmt)){
	 	  echo '<br>';
          echo  $max_order =  $res[0]['max_order'];
		  echo $test;
		    }
			die;*/
/*  $DbExt=new DbExt;
 $startdate = '2016-12-30 12:40';
 $enddate = '2016-12-30 14:40';
 $startdate_pm = '2016-12-30 18:40';
 $enddate_pm = '2016-12-30 22:00';
 $time = '2016-12-30 13:50';
 echo $stc = strtotime($startdate); echo '======>';
 echo $ste = strtotime($enddate); echo '======>';
 
echo  $stcpm = strtotime($startdate_pm); echo '======>';
 echo $stepm = strtotime($enddate_pm); echo '======>';
echo  $t = strtotime($time); echo '======>';
     if((strtotime($startdate) < strtotime($time)) && (strtotime($enddate) > strtotime($time))) 
					{
						echo 'testif';
						 $stmt="SELECT count(order_id) as max_order from
					  {{order}}
				      WHERE merchant_id='$merchant_id' AND 
				      delivery_date = '2016-12-30' AND
					   delivery_time BETWEEN '12:40' AND '14:40'
					  "; 
					}
					  elseif((strtotime($startdate_pm) < strtotime($time)) && (strtotime($enddate_pm) > strtotime($time))) 
					{
						 echo 'elseeeee->';
						 $stmt="SELECT count(order_id) as max_order from
					  {{order}}
				      WHERE merchant_id='$merchant_id' AND 
				      delivery_date = '2016-12-30' AND
					   delivery_time BETWEEN '18:40' AND '22:00'
					  "; 
					 // $res=$DbExt->rst($stmt);
					//  echo '<pre>';
					//  print_r($res);
...
$stores_open_time=Yii::app()->functions->getOption("stores_open_time",$merchant_id);
$stores_ends_time=Yii::app()->functions->getOption("stores_ends_time",$merchant_id);
$stores_open_time_pm=Yii::app()->functions->getOption("stores_open_time_pm",$merchant_id);
$stores_ends_time_pm=Yii::app()->functions->getOption("stores_ends_time_pm",$merchant_id); 
$max_orders=Yii::app()->functions->getOption("max_orders",$merchant_id);
$max_orders_pm=Yii::app()->functions->getOption("max_orders_pm",$merchant_id);
...
$stores_open_time=!empty($stores_open_time)?(array)json_decode($stores_open_time):false;
$stores_ends_time=!empty($stores_ends_time)?(array)json_decode($stores_ends_time):false;
$stores_open_time_pm=!empty($stores_open_time_pm)?(array)json_decode($stores_open_time_pm):false;
$stores_ends_time_pm=!empty($stores_ends_time_pm)?(array)json_decode($stores_ends_time_pm):false;
$max_orders=!empty($max_orders)?(array)json_decode($max_orders):false;
$max_orders_pm=!empty($max_orders_pm)?(array)json_decode($max_orders_pm):false;
...
<div class="uk-form-row">
  ...
  <div class="uk-grid" >
    ...
    <div class="uk-width-1-6" style="width:4%;">
    ...
    <div class="uk-width-1-6" style="width:12%">
      ...
      ,array('placeholder'=>Yii::t("default","Start"),'class'=>"timepick",'readonly'=>"readonly"));?>
    </div>
    
    <div class="uk-width-1-6" style="width:4%;"><?php echo Yii::t("default","To")?></div>
    <div class="uk-width-1-6" style="width:12%">
      ...
      ,array('placeholder'=>Yii::t("default","End"),'class'=>"timepick",'readonly'=>"readonly"));?>
    </div>
  ...
  <li>  <!----- Code for set Max ordder limit --------->
   ...
   <!-- <div class="uk-width-1-6" style="width:4%;">
    ...
    <div class="uk-width-1-6" style="width:16%"><?php echo 'Set '.ucwords(Yii::app()->functions->translateDate($val)).' Order Limit';?></div>    
    
    <div class="uk-width-1-6" style="width:12%">
      <?php echo CHtml::textField("stores_open_time[$key]",
      array_key_exists($key,(array)$stores_open_time)?timeFormat($stores_open_time[$key],true):""
      ,array('placeholder'=>Yii::t("default","Start"),'class'=>"timepick", 'readonly'=>"readonly"));?>
    </div>
    
    <div class="uk-width-1-6" style="width:4%;"><?php echo Yii::t("default","To")?></div>
    <div class="uk-width-1-6" style="width:12%">
      <?php echo CHtml::textField("stores_ends_time[$key]",
      array_key_exists($key,(array)$stores_ends_time)?timeFormat($stores_ends_time[$key],true):""
      ,array('placeholder'=>Yii::t("default","End"),'class'=>"timepick",'readonly'=>"readonly"));?>
    </div>
    
     <div class="uk-width-1-6" style="width:12%">
      <?php echo CHtml::textField("max_orders[$key]",
      array_key_exists($key,(array)$max_orders)?$max_orders[$key]:""
      ,array('placeholder'=>Yii::t("default","MaxOrder")));?>
    </div>
    ...
    <div class="uk-width-1-6" style="width:12%">
      <?php echo CHtml::textField("stores_open_time_pm[$key]",
      array_key_exists($key,(array)$stores_open_time_pm)?timeFormat($stores_open_time_pm[$key],true):""
      ,array('placeholder'=>Yii::t("default","Start"),'class'=>"timepick",'readonly'=>"readonly"));?>
    </div>
    <div class="uk-width-1-6" style="width:4%;"><?php echo Yii::t("default","To")?></div>
    <div class="uk-width-1-6" style="width:12%">
      <?php echo CHtml::textField("stores_ends_time_pm[$key]",
      array_key_exists($key,(array)$stores_ends_time_pm)?timeFormat($stores_ends_time_pm[$key],true):""
      ,array('placeholder'=>Yii::t("default","End"),'class'=>"timepick",'readonly'=>"readonly"));?>
    </div>
    
    <div class="uk-width-1-6" style="width:12%">
      <?php echo CHtml::textField("max_orders_pm[$key]",
       array_key_exists($key,(array)$max_orders_pm)?$max_orders_pm[$key]:""
      ,array('placeholder'=>Yii::t("default","MaxOrder")));?>
    </div>
